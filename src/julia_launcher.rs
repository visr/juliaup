...
fn get_julia_path_from_channel(
    ...
    match channel_info {
        ...
        JuliaupConfigChannel::AliasChannel { target, args: _ } => {
            // For alias channels, we need to recursively resolve to the actual channel
            // This is a simplified implementation - ideally we should resolve the full chain
            bail!(
                "The channel `{}` is an alias to `{}`. Alias resolution in the launcher needs to be implemented.",
                channel,
                target
            );
        }
